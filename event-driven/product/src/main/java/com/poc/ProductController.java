...
public class ProductController {
	...
	@PostMapping("/inquiry")
	public PbkMessage inquiry(@RequestBody PbkMessage event) {
		log.info(String.format("<- %s, %s", "application", event.toString()));

		if (event.getModelType() == ModelType.INDIVIDU_SELECTED) {
			if (event.getUuid() == null)
				event.setUuid(UUID.randomUUID().toString());
			event.setStatus(MessageStatus.ACEPTED);
			event.setMessage("OK");
			service.sendMessage(event, properties.getProductOutToDebiturTopicName());
		} else {
			event.setMessage("need SELECTED_INDIVIDU event model type");
	...
	public void testEvent() {
		for (SseEmitter sseEmitter : emitters) {
			try {
				sseEmitter.send(SseEmitter.event().name("product-orchestrator").data("test"));
			} catch (IOException e) {
				emitters.remove(sseEmitter);
			}
		}
